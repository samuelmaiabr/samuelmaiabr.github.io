...
Nesta seção descrevo brevemente os conceitos e técnicas da disciplina *Ciência dos Dados Avançada* empregados durante a exploração e limpeza dos dados do ***Projeto***. Em poucas palavras, fiz uso intensivo de técnicas como programação funcional, paralelização e manipulação avançada de dados, estruturação e visualização de dados. Cada conceito e técnica é acompanhada de exemplos práticos de código R.

- **Programação Funcional e Criação de Funções:** Encapsulei operações de leitura e transformação em funções específicas para aumentar a reusabilidade e facilitar a manutenção. Utilizei programação funcional com a função personalizada `process_wid_file()`que lê cada arquivo listado por meio de `list.files()` e seleciona e transforma as colunas necessárias. Nesse processo, apliquei conceitos vistos em aula, como orientação a objetos, manipulação avançada de estruturas de dados (especialmente *data frames*), e utilizei funções do pacote `dplyr`, tais como `mutate`, `group_by`.
...
- **Manipulações Avançadas com `dplyr`:** Usei intensivamente verbos como `select`, `filter`, `mutate`, `group_by` e `summarise`, mostrando na prática a aplicação de técnicas vistas no tópico de ***Principais Bibliotecas de Ciência de Dados***.
...
- **Estruturação e Limpeza dos Dados:** Transformei e organizei os dados, convertendo variáveis textuais para numéricas, criando novas variáveis úteis (como `decade` e `period_flag`) e removendo dados faltantes (*NAs*) e valores extremos (*outliers*), alinhando-me com os ***Conceitos de Limpeza e Organização de Dados*** estudados.
